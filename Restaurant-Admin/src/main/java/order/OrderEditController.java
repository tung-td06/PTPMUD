...
public class OrderEditController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // Load ComboBox values and then populate form fields
        loadComboValuesAndPopulate();
    }

    private void loadComboValuesAndPopulate() {
        ...
        new Thread(() -> {
            try {
                ...
                List<OrderDetailModel> existingDetails = new ArrayList<>();
                if (selectedOrder != null) {
                    existingDetails = orderService.getDetailsForOrder(selectedOrder.getMaorder());
                }

                final List<OrderDetailModel> finalExistingDetails = existingDetails;
                ...
                Platform.runLater(() -> {
                    ...
                    if (serverFoods != null) {
                        for (model.MonAn ma : serverFoods) {
                            if (ma.isTrangThai() != null && ma.isTrangThai().equalsIgnoreCase("Còn món")) {
                    ...
                    if (selectedOrder != null) {
                        populateForm(finalExistingDetails);
                    }
                });
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void populateForm(List<OrderDetailModel> existingDetails) {
        txtMaOrder.setText(selectedOrder.getMaorder());
        txtMaHD.setText(selectedOrder.getMahd() != null ? selectedOrder.getMahd() : "");
        dpNgayTao.setValue(selectedOrder.getNgaytao().toLocalDate());
        cbTrangThai.setValue(selectedOrder.getTrangthai());

        // Find and select Bàn
        String tableCode = selectedOrder.getMaban();
        for (String item : cbBanAn.getItems()) {
            if (item.startsWith(tableCode + " - ")) {
                cbBanAn.setValue(item);
                break;
            }
        }

        // Find and select Nhân viên
        String empCode = selectedOrder.getManv();
        if (empCode != null) {
    ...
    private void setupTableColumns() {
        ...
        // Action Column: Delete/change status item button
        colAction.setCellFactory(column -> new TableCell<>() {
            private final Button btn = new Button("Xóa");
            private final Button statusBtn = new Button("Đổi TT");
            {
                ...
                statusBtn.getStyleClass().add("create-btn");
                statusBtn.setStyle("-fx-pref-height: 28; -fx-font-size: 11; -fx-padding: 0 10 0 10; -fx-background-color: #3b82f6;");
                statusBtn.setOnAction(event -> {
                    OrderDetailModel rowData = getTableView().getItems().get(getIndex());
                    String nextStatus = getNextDishStatus(rowData.getTrangthai());
                    
                    int idx = detailsList.indexOf(rowData);
                    if (idx != -1) {
                        detailsList.set(idx, new OrderDetailModel(
                            rowData.getMaorder(),
                            rowData.getMamon(),
                            rowData.getTenmon(),
                            rowData.getSoluong(),
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty) {
                    ...
                    HBox box = new HBox(5, statusBtn, btn);
                    box.setAlignment(Pos.CENTER);
                    setGraphic(box);
                }
            }
    ...
    private String getNextDishStatus(String current) {
        if (current == null) return "Đang chờ";
        return switch (current) {
            case "Đang chờ" -> "Đang chế biến";
            case "Đang chế biến" -> "Đã xong";
            case "Đã xong" -> "Đã phục vụ";
            case "Đã phục vụ" -> "Đã hủy";
            default -> "Đang chờ";
        };
    }
    ...
    private void handleAddItem(ActionEvent event) {
        ...
        for (OrderDetailModel item : detailsList) {
            if (item.getMamon().equalsIgnoreCase(mamon)) {
                ...
                detailsList.add(new OrderDetailModel(txtMaOrder.getText().trim(), mamon, tenmon, currentQty + qty, price, item.getTrangthai()));
                exists = true;
                break;
            }
        }
    ...
    private void handleSave(ActionEvent event) {
        ...
        if (mabanFull == null || manvFull == null || localDate == null || status == null) {
        ...
        // Validate mahd exists if entered
        if (!mahd.isEmpty()) {
        ...
        if ("Hoàn thành".equalsIgnoreCase(status)) {
            ...
            ngaytao = Date.valueOf(localDate);
        }
        ...
        new Thread(() -> {
            try {
                ...
                OrderModel updatedOrder = new OrderModel(selectedOrder.getMaorder(), maban, manv, mahd.isEmpty() ? null : mahd, ngaytao, status);
                List<OrderDetailModel> finalDetailsList = new ArrayList<>();
                for (OrderDetailModel d : detailsList) {
                    finalDetailsList.add(new OrderDetailModel(selectedOrder.getMaorder(), d.getMamon(), d.getTenmon(), d.getSoluong(), d.getDongia(), d.getTrangthai()));
                }

                orderService.updateOrder(updatedOrder, finalDetailsList);

                // Table status sync logic
                String origTable = selectedOrder.getMaban();
                
                if (status.equalsIgnoreCase("Hoàn thành") || status.equalsIgnoreCase("Đã hủy")) {
                    // Release tables
                    List<String> payloadOrig = List.of(origTable, "Trống");
                    SocketClient.getInstance().sendRequest(new Request(Module.BANAN, "UPDATE_TRANG_THAI", payloadOrig));
                    if (!origTable.equalsIgnoreCase(maban)) {
                        List<String> payloadNew = List.of(maban, "Trống");
                        SocketClient.getInstance().sendRequest(new Request(Module.BANAN, "UPDATE_TRANG_THAI", payloadNew));
                    }
                } else {
                    // Occupied tables
                    if (!origTable.equalsIgnoreCase(maban)) {
                    ...
                    List<String> payloadNew = List.of(maban, "Có khách");
                    SocketClient.getInstance().sendRequest(new Request(Module.BANAN, "UPDATE_TRANG_THAI", payloadNew));
                }

                Platform.runLater(() -> {
                    showSuccess("Cập nhật Order thành công!");
                    new Thread(() -> {
