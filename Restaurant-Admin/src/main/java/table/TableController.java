...
public class TableController implements Initializable {
    ...
    private void processServerData(List<BanAn> serverList, List<DatBan> dbList, List<KhachHang> khList,
        ...
        if (serverList != null) {
            ...
            for (BanAn ba : serverList) {
                ...
                if (!hasOccupied) {
                    validBookings.sort(
                            Comparator.comparing(DatBan::getTimeVao, Comparator.nullsLast(Comparator.naturalOrder())));
                    for (DatBan db : validBookings) {
                ...
                new Thread(() -> {
                    try {
                        ...
                        if (responseAdd != null && responseAdd.isSuccess()) {
                            Request requestTable = new Request(Module.BANAN, "UPDATE_TRANG_THAI",
                                    List.of(table.getMaban(), "Đang dùng"));
                ...
                new Thread(() -> {
                    try {
                        if (table.getMaDatBan() != null) {
                            Request requestBooking = new Request(Module.DATBAN, "UPDATE_TRANG_THAI",
                                    List.of(table.getMaDatBan(), "Hoàn thành"));
                            SocketClient.getInstance().sendRequest(requestBooking);
                        }

                        Request requestTable = new Request(Module.BANAN, "UPDATE_TRANG_THAI",
                                List.of(table.getMaban(), "Trống"));
                        SocketClient.getInstance().sendRequest(requestTable);
            ...
            actionBtn.setOnAction(e -> {
                ...
                new Thread(() -> {
                    try {
                        if (table.getMaDatBan() != null) {
                            Request requestBooking = new Request(Module.DATBAN, "UPDATE_TRANG_THAI",
                                    List.of(table.getMaDatBan(), "Đã nhận bàn"));
                            SocketClient.getInstance().sendRequest(requestBooking);
                        }
