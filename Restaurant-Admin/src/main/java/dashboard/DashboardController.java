    ...
    private void loadDashboard() {
        ...
        new Thread(() -> {
            try {
                ...
                final int totalImports = (importList != null) ? importList.size() : restaurant.MockDataStore.imports.size();
                ...
                if (banList != null) {
                    for (BanAn t : banList) {
                        if ("Có khách".equalsIgnoreCase(t.getTrangThai()) || "Đang dùng".equalsIgnoreCase(t.getTrangThai())) {
                            busyTables++;
                        } else if ("Đã đặt".equalsIgnoreCase(t.getTrangThai()) || "Đặt trước".equalsIgnoreCase(t.getTrangThai())) {
                            bookedTables++;
                        }
                    }
                } else {
                ...
                if (serverList != null) {
                    for (HoaDon hd : serverList) {
                        ...
                        LocalDate billDate = hd.getTimeVao() != null ? hd.getTimeVao().toLocalDateTime().toLocalDate() : LocalDate.now();
                    ...
                    for (bill.BillController.BillModel b : restaurant.MockDataStore.bills) {
                        recentList.add(new BillModel(
                                ...
                                b.getPayment(),
                                b.getStatus(),
                                b.getTimeVao() != null ? b.getTimeVao().toLocalDateTime().toLocalDate() : today.minusDays(index++),
                                b.getMaBan()
                        ));
                    }
                }
