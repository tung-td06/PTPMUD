...
public class TableController implements Initializable {
                ...
                Platform.runLater(() -> {
                    if (response != null && response.isSuccess()) {
                        ...
                        if (responseKH != null && responseKH.isSuccess()) {
                            khList = (List<KhachHang>) responseKH.getData();
                        }
                        ...
                        processServerData(serverList, dbList, khList, selectedDate);
                    } else {
    ...
    private void processServerData(List<BanAn> serverList, List<DatBan> dbList, List<KhachHang> khList, LocalDate selectedDate) {
        ...
        if (serverList != null) {
            ...
            for (BanAn ba : serverList) {
                List<DatBan> validBookings = new ArrayList<>();
                if (dbList != null) {
                        ...
                        if (db.getMaBan() != null && db.getMaBan().equalsIgnoreCase(ba.getMaBan())) {
                            if (db.getTimeVao() != null && db.getTimeRa() != null) {
                                ...
                                if (bookingDate.equals(selectedDate)) {
                                    boolean isCancelled = "Đã hủy".equalsIgnoreCase(db.getTrangThai());
                                    boolean isExpired = db.getTimeRa().getTime() < now;
                                    if (!isCancelled && !isExpired) {
                                        validBookings.add(db);
                                    }
                                }
                            }
                        }
                ...
                for (DatBan db : validBookings) {
                    if ("Đã nhận bàn".equalsIgnoreCase(db.getTrangThai())) {
                        hasOccupied = true;
                        matchedBooking = db;
                        break;
                    }
                ...
                if (!hasOccupied) {
                    validBookings.sort(Comparator.comparing(DatBan::getTimeVao, Comparator.nullsLast(Comparator.naturalOrder())));
                    for (DatBan db : validBookings) {
                        if ("Đã xác nhận".equalsIgnoreCase(db.getTrangThai())) {
                            hasReserved = true;
                            matchedBooking = db;
                            break;
                        }
                ...
                if (matchedBooking != null) {
                    model.setMaDatBan(matchedBooking.getMaDatBan());
                    if (computedStatus.equals("Đặt trước")) {
                        String custName = customerMap.getOrDefault(matchedBooking.getMaKH(), matchedBooking.getMaKH());
                        model.setTenKhach(custName);
                        
                        java.sql.Timestamp tv = matchedBooking.getTimeVao();
                        if (tv != null) {
                            java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("HH:mm");
                            model.setGioDen(sdf.format(tv));
                        }
                        model.setSoNguoi(matchedBooking.getSoNguoi());
                    }
                }
        ...
        restaurant.MockDataStore.tables.clear();
        restaurant.MockDataStore.tables.addAll(masterList);
    ...
    private VBox createTableCard(TableModel table) {
        ...
        VBox bookingInfoBox = null;
        if (table.getTrangthai().equalsIgnoreCase("Trống")) {
            card.setPrefSize(180, 165);
            ...
            bookingInfoBox = new VBox(4);
            bookingInfoBox.setAlignment(Pos.CENTER);
            
            String guestName = table.getTenKhach() != null ? table.getTenKhach() : "N/A";
            String arrivalTime = table.getGioDen() != null ? table.getGioDen() : "N/A";
            String guestsStr = table.getSoNguoi() != null ? " (" + table.getSoNguoi() + " NL)" : "";
            
            Label guestLabel = new Label("👤 " + guestName + guestsStr);
            guestLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: #475569; -fx-font-weight: bold;");
            
            Label timeLabel = new Label("⏰ " + arrivalTime);
            timeLabel.setStyle("-fx-font-size: 11px; -fx-text-fill: #ff9800; -fx-font-weight: bold;");
            
            bookingInfoBox.getChildren().addAll(guestLabel, timeLabel);
        }
        ...
        if (table.getTrangthai().equalsIgnoreCase("Trống")) {
            ...
            actionBtn.setOnAction(e -> {
                ...
                new Thread(() -> {
                    try {
                        String newBookingId = "DB" + (System.currentTimeMillis() % 100000000L);
                        ...
                        if (resKH != null && resKH.isSuccess()) {
                            List<KhachHang> listKH = (List<KhachHang>) resKH.getData();
                            if (listKH != null && !listKH.isEmpty()) {
                                customerId = listKH.get(0).getMaKH();
                            }
                        }
                        
                        DatBan newDb = new DatBan();
                        newDb.setMaDatBan(newBookingId);
                        newDb.setMaKH(customerId);
                        newDb.setMaBan(table.getMaban());
                        newDb.setTimeVao(new java.sql.Timestamp(System.currentTimeMillis()));
                        newDb.setTimeRa(new java.sql.Timestamp(System.currentTimeMillis() + 2 * 3600 * 1000L));
                        newDb.setThoiGianDen(new java.sql.Timestamp(System.currentTimeMillis()));
                        newDb.setSoNguoi(1);
                        newDb.setNote("Khách vãng lai");
                        newDb.setTrangThai("Đã nhận bàn");

                        Request requestAdd = new Request(Module.DATBAN, Action.ADD, newDb);
                        Response responseAdd = SocketClient.getInstance().sendRequest(requestAdd);

                        if (responseAdd != null && responseAdd.isSuccess()) {
                            Request requestTable = new Request(Module.BANAN, "UPDATE_TRANG_THAI", List.of(table.getMaban(), "Đang dùng"));
                            SocketClient.getInstance().sendRequest(requestTable);
                        } else {
                            Platform.runLater(() -> showAlert("Lỗi mở bàn: Không thể tạo thông tin đặt bàn!"));
                        }
            ...
            actionBtn.setOnAction(e -> {
                ...
                new Thread(() -> {
                    try {
                        if (table.getMaDatBan() != null) {
                            Request requestBooking = new Request(Module.DATBAN, "UPDATE_TRANG_THAI", List.of(table.getMaDatBan(), "Hoàn thành"));
                            SocketClient.getInstance().sendRequest(requestBooking);
                        }
                        
                        Request requestTable = new Request(Module.BANAN, "UPDATE_TRANG_THAI", List.of(table.getMaban(), "Trống"));
                        SocketClient.getInstance().sendRequest(requestTable);
            ...
            actionBtn.setOnAction(e -> {
                ...
                new Thread(() -> {
                    try {
                        if (table.getMaDatBan() != null) {
                            Request requestBooking = new Request(Module.DATBAN, "UPDATE_TRANG_THAI", List.of(table.getMaDatBan(), "Đã nhận bàn"));
                            SocketClient.getInstance().sendRequest(requestBooking);
                        }
        ...
        if (bookingInfoBox != null) {
            card.getChildren().addAll(statusTag, nameLabel, locationLabel, bookingInfoBox, actionBtn);
        } else {
