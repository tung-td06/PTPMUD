...
public class BillAddController implements Initializable {
    ...
    @FXML
    private ComboBox<String> cbNhanVien;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // Pre-select current time for Time Vao
        java.time.LocalTime now = java.time.LocalTime.now();
        cbTimeVaoHour.setValue(String.format("%02d", now.getHour()));
        cbTimeVaoMin.setValue(String.format("%02d", now.getMinute()));
    ...
    private void loadComboData() {
        ...
        new Thread(() -> {
            try {
                ...
                e.printStackTrace();
                Platform.runLater(() -> {
                    showError("Lỗi tải danh sách ComboBox từ Server: " + e.getMessage());
                    setLoadingState(false);
                });
            }
        }).start();
    ...
    private void handleSave(ActionEvent event) {
        ...
        if (mahd.isEmpty()) {
            showError("Mã hóa đơn bắt buộc phải nhập!");
            return;
        }
        if (mahd.length() > 15) {
            showError("Mã hóa đơn không quá 15 ký tự!");
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                Request addReq = new Request(Module.HOADON, Action.ADD, hd);
                ...
                Platform.runLater(() -> {
                    if (addRes != null && addRes.isSuccess()) {
                        ...
                        showError(addRes != null ? addRes.getMessage() : "Không thể tạo hóa đơn trên Server!");
                    }
                    setLoadingState(false);
                });
