...
public class BookingController implements Initializable {
        ...
        statusColumn.setCellFactory(column -> new TableCell<>() {
            ...
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    if (item.equalsIgnoreCase("Đã xác nhận")) {
                        label.getStyleClass().add("status-confirmed");
                    } else if (item.equalsIgnoreCase("Chờ xác nhận")) {
                    ...
                    if (responseKH != null && responseKH.isSuccess() &&
                        ...
                        if (serverDBList != null) {
                            for (DatBan db : serverDBList) {
                                ...
                                if ("Đã nhận bàn".equals(db.getTrangThai())) {
                                    status = "Đã xác nhận";
                                } else if ("Đã hủy".equals(db.getTrangThai())) {
                                    status = "Đã hủy";
                                }
    ...
    private void updateStatsFiltered(List<BookingModel> list) {
        ...
        for (BookingModel bk : list) {
            if (bk.getStatus().equalsIgnoreCase("Đã xác nhận")) {
                confirmed++;
            } else if (bk.getStatus().equalsIgnoreCase("Chờ xác nhận")) {
                pending++;
            } else {
                cancelled++;
            }
