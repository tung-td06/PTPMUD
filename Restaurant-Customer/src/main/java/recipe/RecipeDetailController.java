    ...
    private void loadDetail() {
        ...
        new Thread(() -> {
            try {
                ...
                // 3. Fetch Ingredients for names and units
                java.util.Map<String, model.NguyenLieu> materialMap = new java.util.HashMap<>();
                Request matReq = new Request(Module.NGUYENLIEU, Action.GET_ALL, null);
                Response matRes = SocketClient.getInstance().sendRequest(matReq);
                if (matRes != null && matRes.isSuccess()) {
                    List<model.NguyenLieu> materials = (List<model.NguyenLieu>) matRes.getData();
                    if (materials != null) {
                        for (model.NguyenLieu nl : materials) {
                            materialMap.put(nl.getMaNL(), nl);
                        }
                    }
                }
 
                // 4. Fetch Recipes matching mamon
                ...
                if (recipeRes != null && recipeRes.isSuccess()) {
                    ...
                    if (recipes != null) {
                        for (model.DinhLuongMon dl : recipes) {
                            if (dl.getMaMon().equalsIgnoreCase(selected.getMamon())) {
